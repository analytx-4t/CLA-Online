...
const { classifyLocal } = require('../guardrails');
...
async function processQuestionFast(q) {
  const start = Date.now();
  try {
    // Fast local guardrail classification
    const localGuard = classifyLocal(q.text);
    let guardrailStatus = 'ALLOWED';
    if (localGuard && localGuard.triggered) {
      guardrailStatus = 'BLOCKED (' + localGuard.category + ')';
    }

    // Prioritized Search & Cohere Rerank
    const results = await performPrioritizedLegalSearch(q.text, q.text);
    const elapsed = Date.now() - start;
    const tables = [...new Set(results.map(r => r.source_table))];
    const topTitle = results[0]?.doc_title || results[0]?.law_title || results[0]?.versus || 'N/A';
    const topSec = results[0]?.sections || 'N/A';
    ...
    return {
      ...
      tables: tables,
      topSource: `${results[0]?.source_table || 'N/A'}: ${(topTitle).substring(0, 35)} (Sec: ${topSec})`
    };
  } catch (err) {
    const elapsed = Date.now() - start;
    return {
...
async function runFastBenchmark() {
  console.log(`\n========================================================================================`);
  console.log(`BENCHMARKING ALL 20 QUESTIONS FROM CLA_Online_Before_After_Comparison.docx IN PARALLEL`);
  console.log(`========================================================================================\n`);
  ...
  for (let i = 0; i < questions.length; i += BATCH_SIZE) {
    ...
    console.log(`[Batch ${batchNum}/4] Processing questions ${batch[0].id} - ${batch[batch.length - 1].id} in parallel...`);
    const batchResults = await Promise.all(batch.map(q => processQuestionFast(q)));
  ...
  console.log(`\n========================================================================================`);
  console.log(`RESULTS SUMMARY: Completed 20 Legal Questions in ${(totalElapsed / 1000).toFixed(2)} seconds`);
  console.log(`========================================================================================\n`);
  ...
  console.table(allResults.map(r => ({
    ...
    "Time (ms)": r.elapsedMs,
    "Chunks": r.chunksFound,
    "Source Tables": r.tables.join(', '),
    "Top Retrieved Reference": r.topSource
  })));

  const blocked = allResults.filter(r => r.guardrail.startsWith('BLOCKED'));
  console.log(`\nTotal Passed Guardrails: ${allResults.length - blocked.length} / ${allResults.length}`);
  console.log(`Total False Positive Blocked: ${blocked.length}`);
